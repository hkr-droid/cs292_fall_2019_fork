...
(define (step ς)
  (match ς
    ...
    [(state `(,f . ,es) ρ σ κ)
     ...
     ;; todo feel like you'd need to update the store here, or in the above
     ;; eval-atom, since the argument is the next command, which needs the
     ;; modified state; could maybe do just by pattern matching on the
     ;; command type?
...
;; command examples
;; ite  <-- I think this is wrong though; currently seems to do big-step
(define prog4
  `(if (and #t #f)
       (+ 3 5)
       (+ 3 4)))
